...
public class Octopus : BossMonster
{
    ...
    */
        ...
        if (PhotonNetwork.IsMasterClient && GM.target_count != 1)
        {
            ...
            if (_ran == 0)
            {
                while (true)
                {
                    ...
                    if(_r != target_index)
                    {
                        pv.RPC("Change_target_RPC", RpcTarget.All, _r);
                        break;
                    }
                }
            }
    ...
    {
        target = GM.targetsDic[GM.targets[index]];
    ...
    [PunRPC]
    private void Change_target_RPC(int index)
    {
        target = GM.targetsDic[GM.targets[index]];
        target_index = index;
    }

    private int Gen_target_index()
    {   
        while (true)
        {
    ...
    [PunRPC]
    private void Hit_RPC(int damage, Vector3 point)
    {
        curHp -= damage;
    ...
    public void View_Reward_UI()
    {
        ...
        {
            rewardController.AddReward(rewardNames[i], rewardCounts[i]);
            CFirebase.Instance.GetItem(rewardNames[i], rewardCounts[i]);
        }
    }
