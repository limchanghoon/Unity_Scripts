    ...
    private void Update()
    {
        ...
        else if (Input.GetKeyDown(KeyCode.Q))
        {
            if (PhotonNetwork.InRoom)
                PhotonNetwork.Instantiate("Mini Rocket", fire_pos.position, theCam.transform.rotation);
            else
                Instantiate(miniRocketPrefab, fire_pos.position, theCam.transform.rotation);
        }
        else if (canPortalGun && Input.GetKeyDown(KeyCode.Alpha1))
        {
            if (PhotonNetwork.InRoom)
        ...
        else if (canPortalGun && Input.GetKeyDown(KeyCode.Alpha2))
        {
            if (PhotonNetwork.InRoom)
