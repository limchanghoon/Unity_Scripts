...
public class InventoryController : MonoBehaviour
{
    ...
    public int Find_ID_From_UUID(string _uuid)
    {
        for (int i = 0; i < equipments.Count; i++)
        {
            if (equipments[i].uuid == _uuid)
                return equipments[i].id;
        }
        return -1;
    }

    public int Find_Count_From_Name(string _Name)
    {
        var item = Find_Item(_Name);
